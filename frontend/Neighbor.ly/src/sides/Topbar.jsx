...
const Topbar = () => {
  ...
  return (
    <div className='flex  top-0 w-full items-center justify-between py-4 px-6 border-b-2 border-gray-400'>
      <div>
        <h1 className='text-center font-bold text-lg sm:text-2xl'>Welcome back, {user.name}! </h1>
          ...
          <button onClick={() => setnotificationOpen(!notificationOpen)} className='flex p-2 hover:bg-gray-300 rounded-full cursor-pointer'>
            ...
            <div className={`fixed bg-gray-100 z-10 overflow-hidden right-5 origin-top-right transition-all ease-in-out duration-300 ${notificationOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'} top-28 p-2 w-72 md:w-md lg:w-2xl border border-gray-400 rounded-lg`}> 
